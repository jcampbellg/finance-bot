...
export async function botTransaction(params: ConversationPropsWithBookSelected, t: TransactionWithAll, editMessage: boolean = false) {
  ...
  const keyboard = [
    ...
    [{ text: '💵 Cambiar Monto', callback_data: `transaction_amount_${t.id}` }, { text: '📅 Cambiar Fecha', callback_data: isNormal ? `transaction_paid_date_${t.id}` : `transaction_date_${t.id}` }],
