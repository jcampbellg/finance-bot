  ...
  await bot.sendMessage(chatId, `Editando Transacción\n\n<b>Descripción:</b> ${t.description}\n<b>Monto:</b> ${amount}\n<b>Fecha:</b> ${spanishDate}\n<b>Cuenta:</b> ${t.account.description}\n<b>${categoryLabel}</b> ${category}${isPaidLabel}`, {
    ...
    reply_markup: {
      inline_keyboard: [
        ...
        [...(isNormal ? [{ text: `🏷️ Categoría`, callback_data: `transaction_category_${t.id}` }] : []), { text: '💵 Cambiar Monto', callback_data: `transaction_amount_${t.id}` }],
        ...
        [{ text: '📎 Adjuntar', callback_data: `transaction_file_${t.id}` }],
        menuBtn
      ]
    }
  })
