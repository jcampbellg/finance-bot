...
bot.on('message', async (msg) => {
  ...
  if (user.chatSubject === 'category') {
    ...
    if (user.chatSubSubject === 'view') {
      if (userText.startsWith('/')) {
        try {
          const index = parseInt(userText.replace('/', ''))
  ...
  if (userText === '/resumen') {
    ...
    const currentStatement = await prisma.statement.findUnique({
      where: {
        id: user.statementIdSet
      }
    })
    ...
    if (!currentStatement) {
      await bot.sendMessage(msg.chat.id, 'No se encontró el estado de cuenta.')
      return
    }
    ...
    const summary = allCategories.map((category, i) => {
      const isUSD = category.currency === 'USD'

      const total = category.transactions.reduce((acc, transaction) => {
        const isSameCurrency = transaction.currency === category.currency

        const amount = isSameCurrency ? transaction.amount : (isUSD ? transaction.amount * dollarToHNL : transaction.amount * hnlToDollar)

        if (transaction.type === 'EXPENSE') return acc + amount
        return acc - amount
      }, 0)

      return {
        text: `<b>/${i + 1}. ${category.description}</b>\n${numeral(total).format('0,0.00')} ${category.currency}`,
        currency: category.currency,
        total
      }
    })

    const totalInUSD = summary.reduce((acc, category) => {
      if (category.currency === 'HNL') {
        return acc + (category.total * hnlToDollar)
      }
      return acc + category.total
    }, 0)

    const totalInHNL = summary.reduce((acc, category) => {
      if (category.currency === 'USD') {
        return acc + (category.total * dollarToHNL)
      }
      return acc + category.total
    }, 0)
    ...
    const monthInSpanish = dayjs().locale('es').month(currentStatement.month - 1).format('MMMM')
    const year = currentStatement.year

    await bot.sendMessage(msg.chat.id, `<i>Presiona /# para ver las transaciones.</i>\nResumen de ${monthInSpanish} ${year}:\n\n${summaryText}\n\nTotal HNL: ${numeral(totalInHNL).format('0,0.00')}\nTotal USD: ${numeral(totalInUSD).format('0,0.00')}`, { parse_mode: 'HTML' })
    return
  }

  if (user.chatSubject === 'summary') {
    if (userText.match(/\/\d+/)) {
      const index = parseInt(userText.replace('/', '')) - 1
      const category = await prisma.category.findFirst({
      ...
      if (category.transactions.length === 0) {
        await bot.sendMessage(msg.chat.id, 'No hay transacciones registradas.')
        return
      }

      const transMap = category.transactions.map((transaction, i) => {
        return `/${i}. ${transaction.description}\n${numeral(transaction.amount).format('0,0.00')} ${transaction.currency}`
      })
